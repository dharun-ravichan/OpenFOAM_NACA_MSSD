...
| `convergence_study.py` | Entry point 1: sweeps surface point count N at fixed camber, produces `convergence_results.csv` and a Cl/Cd-vs-N plot. |
| `camber_study.py` | Entry point 2: sweeps camber M = 0-8 at fixed N, produces `camber_results.csv` and a Cl/Cd-vs-camber plot. |
| `check_sim_time_convergence_v2.py` | Diagnostic: scans a completed case's saved time series to find the simulation-time convergence point. |
| `main.qmd` | Project report (Quarto). |
